...
public class OrderDetailsActivity extends AppCompatActivity implements ActivityOrderDetailsView {
    ...
    public void onSuccess(SingleOrderModel data) {
        ...
        double total = Double.parseDouble(orderModel.getTotal_cost())+25.0;
        if (orderModel.getBill_currency().equals("eg")){
            binding.tvTotalCost.setText(String.format(Locale.ENGLISH, "%s %s",total,getString(R.string.tl)));


        }else {
            binding.tvTotalCost.setText(String.format(Locale.ENGLISH, "%s %s",total,getString(R.string.aed)));
